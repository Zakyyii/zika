...
const groupListPath = './groupList.json'
...
module.exports = async (client, ctx) => {
   ...
   try {
      ...
      cron.schedule('00 00 * * *', () => {
         setting.lastReset = new Date * 1
         global.db.users.filter(v => v.limit_antilink < env.limit_antilink).map(v => v.limit_antilink = 0)
         Object.entries(global.db.statistic).map(([_, prop]) => prop.today = 0)
      }, {
         scheduled: true,
         timezone: process.env.TZ
...
if (setting.autojpm) {
// Function to read and parse the group list JSON file
const readGroupList = () => {
    return JSON.parse(fs.readFileSync(groupListPath, 'utf8'));
};

// Function to update the group list JSON file
const updateGroupList = (groupList) => {
    fs.writeFileSync(groupListPath, JSON.stringify(groupList, null, 2), 'utf8');
};

// Function to check the group list and send messages
const checkAndSendMessages = () => {
    const groupList = readGroupList();
    let daerah = process.env.TZ
    const now = moment().tz('Asia/Jakarta');

    Object.keys(groupList).forEach(groupId => {
        const group = groupList[groupId];
        if (group.time === now.format('HH:mm')) {

let pesan = `*⚜️LIST PROMO HARGA VPS BY Zaxskyy⚜️*
...
*© Zaxskyy HOSTING*`
let chat_owner = `https://wa.me/6285183123144`
let testimoni = `https://whatsapp.com/channel/0029VaZvgc0GZNCnn3VPXs2G`
let cover = `` // isi kalau mau ada gambar
...
var buttons = [{
   ...
   buttonParamsJson: JSON.stringify({
      ...
      url: testimoni,
      merchant_url: testimoni
   })
}]
client.sendIAMessage(groupId, buttons, null, {
   header: '',
   content: pesan,
   footer: '',
   media: cover
            ...
            // Update the time by adding 5 minutes
            const newTime = now.add(1, 'minutes').format('HH:mm');
            group.time = newTime;
        }
    });

    // Save the updated group list
    updateGroupList(groupList);
};
// Schedule the job to run every 5 seconds
schedule.scheduleJob('*/5 * * * * *', checkAndSendMessages);
}
